...
public class TestActionTest {
	...
	public void testPersonActionList() throws Exception {
		...
		MvcResult rs = mockMvc.perform(get("/remburse/querylist")).andExpect(status().isOk()).andDo(print())
				.andReturn();
		PageInfo<ReimburseVo> info=(PageInfo<ReimburseVo>) rs.getModelAndView().getModel().get("pageinfo");
		for (ReimburseVo air : info.getList()) {
			System.out.println(air.getEmployeename());
		}
