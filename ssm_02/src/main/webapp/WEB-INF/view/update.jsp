...
<body>
	<div id="content">
				...
				<form action="/ssm_02/user/update" class="form" method="post" enctype="multipart/form-data">
				<input type="hidden" name="createdBy" value="${user.userName }">
				<input type="hidden" name="pid" value="${paper.pid }">
					<p>论文题目：<input name="title" onblur="kong()" value=${paper.title } /><span id="title"></span></p>
					<p>论文类型：<select name="type">
									<option>--请选择--</option>
									<c:forEach items="${listcode }" var="code">
										<option value= ${code.cid } ${paper.type==code.cid?"selected":"" }>${code.codeTypeName}</option>
									</c:forEach>
							</select>
					</p>
					<p>论文摘要：<textarea name="paperSummary" cols="30" rows="5" onblur="kong()">${paper.paperSummary }</textarea><span id="paperSummary"> </span></p>
					<p>论文内容：<input type="file" name="imgFile" value="${paper.paperPath }"/><span></span></p>
